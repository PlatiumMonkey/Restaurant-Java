...
import com.aldrin.cafsysv2.gui.dialog.au.JDialogStaffAU;
 ...
 * @author ALDRIN B. C.
 */
public class JDialogStaff extends javax.swing.JDialog implements MouseListener {
    ...
    public JDialogStaff(JFrameCafSys jFrameCafSys, boolean modal) {
    ...
    private void jButtonNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonNewActionPerformed
        ...
        JDialogStaffAU staffAU = new JDialogStaffAU(jFrameCafSys, true);
        staffAU.setVisible(true);
        selectStaff();
...
//        selectUserAccount();
        JDialogStaffAU staffAU = new JDialogStaffAU(jFrameCafSys, true, staff, "Update");
        staffAU.setVisible(true);
        jButtonUpdate.setEnabled(false);
    ...
    public DefaultTableModel tableModel = new DefaultTableModel(new Object[]{"ID", "NAME", "GENDER", "DOB", "PHONE NUMBER", "EMAIL", "POSITION"}, 0) {
    ...
    private void setTable() {
        jTableStaff.setCellSelectionEnabled(true);
        jTableStaff = new JTable(tableModel);
        jScrollPane1.setViewportView(jTableStaff);
        jTableStaff.addMouseListener(this);
        jTableStaff.setRowSorter(sorter);
        TableColumn hide0 = jTableStaff.getColumnModel().getColumn(0);
        hide0.setMinWidth(0);
        hide0.setMaxWidth(0);
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == jTableStaff) {
            if (e.getButton() == MouseEvent.BUTTON1) {
                if (e.getClickCount() == 1) {
                    int row = jTableStaff.getSelectedRow();

                    if (row != -1) {
                        Long id = Long.parseLong(jTableStaff.getValueAt(row, 0).toString());
                        
                        String name = jTableStaff.getValueAt(row, 1).toString();
                        String gender = jTableStaff.getValueAt(row, 2).toString();
                        String dob = jTableStaff.getValueAt(row, 3).toString();
                        String phoneNumber = jTableStaff.getValueAt(row, 4).toString();
                        String email = jTableStaff.getValueAt(row, 5).toString();
                        String position = jTableStaff.getValueAt(row, 6).toString();
...
//"ID", "ROLE ID", "FIRST NAME", "SURNAME", "ROLE", "USERNAME", "PASSWORD", "ACTIVE"
    ...
    private void selectStaff() {
        ...
        staffList = staffDAOImpl.selectStaff();
        tableModel.setRowCount(0);
        for (Staff c : staffList) {
            tableModel.addRow(new Object[]{c.getId(), c.getName(), c.getGender(), c.getDob(), c.getPhoneNumber(),c.getEmail(), c.getPosition()});
        }
    }
